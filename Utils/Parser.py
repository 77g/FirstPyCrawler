...
class Parser:
    ...
    def parse_gzh(text):
        ...
        lists = page.xpath('//ul[@class="news-list2"]/li')
        result = []
        for item in lists:
            url = Parser.get_element(item, 'div/div[1]/a/@href')
            info = Parser.get_elem_text(Parser.get_element(item, 'div/div[2]/p[2]'))
            qrcode = Parser.get_element(item, 'div/div[3]/span/img[1]/@src')
            result.append({'profile_url': url,
    ...
    @staticmethod
    def parse_article(text):  # this is unused
        return []
    ...
    def parse_urls_from_profile(content):
        ...
        for article in articles['list']:
            ...
            result.append({
                ...
                'content_url': 'https://mp.weixin.qq.com' + Parser.__replace_str_html(app_msg_ext_info.get('content_url')),  # RT special
        ...
        return [item for item in result if item['content_url'] != '']
